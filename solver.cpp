						 ...
						 const size_t &max_omp)
{
	...
	#pragma omp parallel num_threads(max_omp) 
	{
		...
		{
			MPI_Send(&L_V[l], 1, MPI_DOUBLE, num_mpi - 1, MY_TAG, MPI_COMM_WORLD);
			MPI_Send(&B_V[l], 1, MPI_DOUBLE, num_mpi - 1, MY_TAG, MPI_COMM_WORLD);		
			MPI_Send(&R_V[l], 1, MPI_DOUBLE, num_mpi - 1, MY_TAG, MPI_COMM_WORLD);
			MPI_Send(&F_V[l], 1, MPI_DOUBLE, num_mpi - 1, MY_TAG, MPI_COMM_WORLD);	
		}
	
		if( num_mpi != max_mpi-1 && num_omp == (max_omp - 1) ) {				
			MPI_Recv(L_V.data()+r+1, 1, MPI_DOUBLE, num_mpi + 1, MY_TAG, MPI_COMM_WORLD, &status);
			MPI_Recv(B_V.data()+r+1, 1, MPI_DOUBLE, num_mpi + 1, MY_TAG, MPI_COMM_WORLD, &status);
			MPI_Recv(R_V.data()+r+1, 1, MPI_DOUBLE, num_mpi + 1, MY_TAG, MPI_COMM_WORLD, &status);
			MPI_Recv(F_V.data()+r+1, 1, MPI_DOUBLE, num_mpi + 1, MY_TAG, MPI_COMM_WORLD, &status);
		}
		...
		if(num_omp == 0) {
			for(int i=0; i < max_mpi; i++)
				if (i != num_mpi) {
					MPI_Send(BUFFER_A.data() + num_mpi*max_omp, max_omp, MPI_DOUBLE, i, MY_TAG, MPI_COMM_WORLD);
					MPI_Send(BUFFER_B.data() + num_mpi*max_omp, max_omp, MPI_DOUBLE, i, MY_TAG, MPI_COMM_WORLD);		
					MPI_Send(BUFFER_C.data() + num_mpi*max_omp, max_omp, MPI_DOUBLE, i, MY_TAG, MPI_COMM_WORLD);
					MPI_Send(BUFFER_F.data() + num_mpi*max_omp, max_omp, MPI_DOUBLE, i, MY_TAG, MPI_COMM_WORLD);	
				}
		
			for(int i=0; i<max_mpi; i++)
				if (i != num_mpi) {
					MPI_Recv(BUFFER_A.data() + max_omp*i, max_omp, MPI_DOUBLE, i, MY_TAG, MPI_COMM_WORLD, &status);
					MPI_Recv(BUFFER_B.data() + max_omp*i, max_omp, MPI_DOUBLE, i, MY_TAG, MPI_COMM_WORLD, &status);		
					MPI_Recv(BUFFER_C.data() + max_omp*i, max_omp, MPI_DOUBLE, i, MY_TAG, MPI_COMM_WORLD, &status);
					MPI_Recv(BUFFER_F.data() + max_omp*i, max_omp, MPI_DOUBLE, i, MY_TAG, MPI_COMM_WORLD, &status);	
				}
		}
		#pragma omp barrier
#ifdef TEST 
			...
			if(num_omp == 0) {
				for(size_t i=0; i<max_mpi; i++)
					if( i != num_mpi) {
						...
						MPI_Send(ANSWER.data() + l_, r_-l_, MPI_DOUBLE, i, MY_TAG, MPI_COMM_WORLD);
				}
